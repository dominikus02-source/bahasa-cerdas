...
export async function generateRPPDocx(input: RPPInput): Promise<Buffer> {
  ...
  if (hasRenderableText(editable)) {
    ...
    const mdDoc = new Document({
      title: input.title,
      description: "Rencana Pembelajaran generated by BahasaCerdas AI",
      styles: {
        default: {
          document: {
  ...
  sections.push(
    new Paragraph({
      children: [new TextRun({ text: "RENCANA PEMBELAJARAN", bold: true, size: 28 })],
  ...
  const identityRows = [
    ...
    { label: "Kurikulum", value: getStr(identity, "curriculum", "Kurikulum Nasional") },
