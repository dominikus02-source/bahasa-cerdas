...
export function createCanvasRenderer(
  ...
  /**
   * Render the Desa ground using the original prototype's visual grammar.
   *
   * IMPORTANT: gameplay still owns the canonical 46x36 tile grid. This pass
   * only changes presentation. The prototype used a 16px visual vocabulary;
  ...
  function renderTiles(state: RPGGameState, camera: RPGCameraState) {
    ...
    for (let y = 0; y < tiles.height; y += 1) {
      for (let x = 0; x < tiles.width; x += 1) {
        ...
        if (sx.x < -tilePx || sx.y < -tilePx || sx.x > width + tilePx || sx.y > height + tilePx) continue;
        ...
        if (bound) {
          ...
          ctx.drawImage(bound as unknown as CanvasImageSource, sx.x - drawW / 2, sx.y - drawH / 2, drawW, drawH);
        ...
        const tileNum = Number(tileId.split(".")[1]);
        const color = tileNum === 1 ? COLORS.path : tileNum === 3 ? COLORS.water : tileNum === 13 ? "#a0845c" : COLORS.grass;
