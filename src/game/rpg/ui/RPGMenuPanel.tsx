          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {inventory.length ? inventory.map((item) => {
                ...
                return <div key={item.itemId} className="rounded-2xl border border-white/10 bg-white/5 p-3"><div className="flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl bg-black/20">{def && EQUIPMENT.some((e) => e.id === item.itemId) ? <img src={equipmentIconFor(item.itemId)} alt="" aria-hidden width={40} height={40} className="object-contain" /> : icon?.status === "READY" ? <img src={icon.path} alt="" aria-hidden width={40} height={40} className="object-contain" /> : <span aria-hidden className="text-lg">✦</span>}</div><div><div className="text-sm font-black">{def?.name ?? item.itemId}</div><div className="mt-1 text-xs text-amber-300">×{item.quantity}</div>{def && EQUIPMENT.some((e) => e.id === item.itemId) ? <div className="mt-1 text-[10px] text-stone-400">{Object.entries(EQUIPMENT.find((e) => e.id === item.itemId)?.modifiers ?? {}).map(([k,v]) => `+${v} ${k.toUpperCase()}`).join(" · ")}</div> : null}</div></div><div className="mt-2 flex items-center justify-between gap-2"><div className="text-[10px] uppercase tracking-wider text-stone-500">{def?.kind ?? "item"}</div>{def && EQUIPMENT.some((e) => e.id === item.itemId) ? <button type="button" onClick={() => onEquip(item.itemId)} className="rounded-lg border border-amber-300/30 bg-amber-400/15 px-2 py-1 text-[10px] font-black text-amber-200 hover:bg-amber-400/25">{equipment.weaponId === item.itemId || equipment.armorId === item.itemId || equipment.accessoryId === item.itemId ? "Lepas" : "Pakai"}</button> : null}</div></div>;
