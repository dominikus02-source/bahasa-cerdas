...
export default function LariKataGame({ hideBackButton, backHref = "/arena/game" }: { hideBackButton?: boolean; backHref?: string }) {
  ...
  const chunky = "border-4 border-[#161B3A] dark:border-white/25 shadow-[6px_6px_0_#EA580C]";
  ...
  if (screen === "start") {
    return (
      <div className="game-env game-env-lari fixed inset-0 z-[60] overflow-y-auto game-env-bg bg-gradient-to-b from-[#FFF6E0] to-[#FFE2C7] dark:from-[#150C06] dark:to-[#201008] text-[#161B3A] dark:text-[#F1EDFF]">
        ...
        <div className="pointer-events-none fixed top-[8%] left-[3%] w-16 h-16 bg-[#F59E0B] border-4 border-[#161B3A] dark:border-white/25 rounded-3xl" style={{ animation: "lk-float1 9s ease-in-out infinite" }} />
        <div className="pointer-events-none fixed top-[16%] right-[5%] w-12 h-12 bg-[#FF6B6B] border-4 border-[#161B3A] dark:border-white/25 rounded-full" style={{ animation: "lk-float2 10s ease-in-out infinite" }} />
        <div className="pointer-events-none fixed bottom-[14%] left-[2%] w-14 h-14 bg-[#38BDF8] border-4 border-[#161B3A] dark:border-white/25 rounded-2xl" style={{ animation: "lk-float1 11s ease-in-out infinite" }} />
        <div className="pointer-events-none fixed bottom-[10%] right-[4%] w-11 h-11 bg-[#4ADE80] border-4 border-[#161B3A] dark:border-white/25 rounded-[30%_70%_70%_30%]" style={{ animation: "lk-float2 8s ease-in-out infinite" }} />
        ...
        <div className="relative max-w-xl mx-auto px-4 py-5 min-h-full flex flex-col">
          ...
          <div className="lk-screen game-env-card bg-white dark:bg-gradient-to-br dark:from-[#221810] dark:to-[#2E2218] rounded-3xl p-6 text-center flex-1 flex flex-col items-center justify-center">
            ...
            <div className="grid grid-cols-3 gap-2.5 mb-6 w-full max-w-xs">
              <div className="bg-[#4ADE80] border-[3px] border-[#161B3A] dark:border-white/25 rounded-xl p-2 shadow-[3px_3px_0_#EA580C]">
                <div className="text-[10px] font-extrabold uppercase opacity-70">Benar</div>
                <div className="font-extrabold text-lg">+100</div>
              </div>
              <div className="bg-[#FBBF24] border-[3px] border-[#161B3A] dark:border-white/25 rounded-xl p-2 shadow-[3px_3px_0_#EA580C]">
                <div className="text-[10px] font-extrabold uppercase opacity-70">Rentetan</div>
                <div className="font-extrabold text-lg">Bonus</div>
              </div>
              <div className="bg-[#FF6B6B] text-white border-[3px] border-[#161B3A] dark:border-white/25 rounded-xl p-2 shadow-[3px_3px_0_#EA580C]">
      ...
      <div className="game-env game-env-lari fixed inset-0 z-[60] overflow-y-auto bg-gradient-to-b from-[#FFF6E0] to-[#FFE2C7] dark:from-[#150C06] dark:to-[#201008] text-[#161B3A] dark:text-[#F1EDFF]">
        <div className="relative max-w-lg mx-auto px-5 pt-4 pb-8 min-h-full flex flex-col">
          <div className="flex items-center justify-between mb-3">
            {!hideBackButton && (
              <GameBackButton onClick={() => { stopBGM(); setScreen("start"); }} label="Kembali ke Lari Kata" title="Kembali ke menu Lari Kata" />
            )}
            <div className={`flex items-center gap-2 ${hideBackButton ? "ml-auto" : ""}`}>
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white dark:bg-[#221810] border-2 border-[#161B3A] dark:border-white/25">
          ...
          <div className="h-2.5 rounded-full bg-white dark:bg-[#221810] border-2 border-[#161B3A] dark:border-white/25 overflow-hidden mb-5">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={currentQ} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.18 }}
              className="bg-white dark:bg-[#221810] rounded-2xl p-5 mb-5 border-4 border-[#161B3A] dark:border-white/25 shadow-[5px_5px_0_#EA580C]">
          ...
          <div className="space-y-2.5 flex-1">
            {q.options.map((opt, i) => {
              ...
              if (feedback) {
                ...
                else style = "bg-white/50 dark:bg-white/5 text-[#161B3A] dark:text-[#F1EDFF]/30";
              }
              return (
                <button key={i} onClick={() => handleAnswer(i)} disabled={feedback !== null}
                  className={`w-full text-left px-4 py-3.5 rounded-2xl border-[3px] border-[#161B3A] dark:border-white/25 font-semibold transition-all active:scale-[0.98] shadow-[3px_3px_0_#EA580C] flex items-center justify-between ${style}`}>
                  <span>{opt}</span>
  ...
  return (
    <div className="game-env game-env-lari fixed inset-0 z-[60] overflow-y-auto bg-gradient-to-b from-[#FFF6E0] to-[#FFE2C7] dark:from-[#150C06] dark:to-[#201008] text-[#161B3A] dark:text-[#F1EDFF]">
      <div className="relative max-w-xl mx-auto px-4 py-5 min-h-full flex flex-col items-center justify-center text-center">
        ...
        <div className="bg-white dark:bg-[#221810] rounded-2xl p-5 mb-4 border-4 border-[#161B3A] dark:border-white/25 shadow-[5px_5px_0_#EA580C] w-full max-w-sm">
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="bg-[#4ADE80] border-[3px] border-[#161B3A] dark:border-white/25 rounded-xl p-2 shadow-[2px_2px_0_#EA580C]">
              <div className="text-[9px] font-extrabold uppercase opacity-70">Benar</div>
              <div className="font-extrabold text-lg">{correct}</div>
            </div>
            <div className="bg-[#FF6B6B] text-white border-[3px] border-[#161B3A] dark:border-white/25 rounded-xl p-2 shadow-[2px_2px_0_#EA580C]">
              <div className="text-[9px] font-extrabold uppercase opacity-70">Salah</div>
              <div className="font-extrabold text-lg">{wrong}</div>
            </div>
            <div className="bg-[#FBBF24] border-[3px] border-[#161B3A] dark:border-white/25 rounded-xl p-2 shadow-[2px_2px_0_#EA580C]">
        ...
        {submitting ? (
          ...
          <div className="bg-emerald-100 dark:bg-emerald-500/15 border-4 border-[#161B3A] dark:border-white/25 rounded-2xl p-4 mb-4 shadow-[4px_4px_0_#EA580C] w-full max-w-sm">
        ...
        <div className="flex gap-3 w-full max-w-sm">
          ...
          {!hideBackButton && (
            <a href={backHref} className={`${btnBase} flex-1 py-3.5 bg-orange-500/20 border-2 border-orange-400/40 text-center`}>
              Menu
            </a>
          )}
        </div>
