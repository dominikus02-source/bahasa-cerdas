...
import {
  Brain, Users, Search, AlertTriangle, Download,
  Loader2, Sparkles, CheckCircle2, TrendingUp, GraduationCap,
} from "lucide-react"
...
/**
 * HasilSimulasiView — tabel rekap hasil simulasi UKBI/TKA + Insight AI + Ringkasan Kelas.
 *
 * Dipakai oleh 2 konteks:
 * - Route legacy /guru/hasil-simulasi (standalone, hero besar).
 * - Hub /guru/evaluasi-simulasi?tab=hasil (hub = true, hero disembunyikan,
 *   cross-link memakai ?tab= agar tetap di dalam hub).
 */
export function HasilSimulasiView({ guruName, hub = false }: { guruName: string; hub?: boolean }) {
  ...
  const dokumenHref = hub ? "?tab=dokumen" : "/guru/dokumen-latihan"
  ...
  return (
    <div className={hub ? "mx-auto max-w-6xl" : "mx-auto max-w-6xl p-4 sm:p-6"}>
                  ...
                  <tr key={`${r.userId}-${r.paketId}`} className="border-b border-slate-50 hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-1">
                        {r.pendingReview > 0 && (
                          <Link href={tinjauHref} className="text-xs px-2 py-1 rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 font-semibold whitespace-nowrap">Tinjau</Link>
                        )}
                        {r.hasCert && (
                          <Link href={dokumenHref} className="text-xs px-2 py-1 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 font-semibold whitespace-nowrap">Dokumen</Link>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {/* CTA ke tab lain di dalam hub / halaman lain di standalone */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
        <Link href={tinjauHref} className="bg-white rounded-2xl border border-violet-200 p-4 hover:bg-violet-50/50 transition-colors">
        ...
        <Link href={dokumenHref} className="bg-white rounded-2xl border border-emerald-200 p-4 hover:bg-emerald-50/50 transition-colors">
