    ...
    <GlassCard className="p-4">
      ...
      {visibleRest.length > 0 && (
        <div className="space-y-1.5">
          {visibleRest.map((e) => (
            <div key={e.userId} className={`flex items-center gap-3 rounded-xl px-3 py-2 transition-colors ${e.isMe ? "border border-[var(--px-gold)]/40 bg-[var(--px-gold)]/10" : "bg-[var(--px-glass)] hover:bg-[var(--px-glass-strong)]"}`}>
