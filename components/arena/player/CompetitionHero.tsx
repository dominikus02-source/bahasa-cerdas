  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
      ...
      <div className="mx-4 mt-4 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 text-white p-4 flex items-center gap-4">
        <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-lg font-extrabold border-2 border-white/30 shrink-0">
          {my ? `#${my.rank}` : "—"}
        </div>
        ...
        <div className="mx-4 mt-4 grid grid-cols-3 gap-2">
          {[1, 0, 2].map((idx) => {
            ...
            return (
              <div
                key={e.userId}
                className={`rounded-xl border p-3 text-center ${isFirst ? "bg-amber-50 border-amber-200" : "bg-gray-50 border-gray-100"}`}
              >
                <p className="text-xl">{MEDALS[idx]}</p>
                <p className="font-bold text-sm text-gray-900 truncate mt-1">{nameOf(e)}</p>
                <p className="text-[11px] text-gray-500 mt-0.5">{e.score.toLocaleString("id-ID")} XP</p>
              </div>
            );
          })}
        </div>
          ...
          {above && (
            <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-2.5 flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="text-[11px] text-emerald-700 font-semibold truncate">#{above.rank} {nameOf(above)}</p>
                <p className="text-[10px] text-emerald-600">{above.weeklyXp.toLocaleString("id-ID")} XP</p>
              </div>
            </div>
          )}
          {below && (
            <div className="rounded-xl bg-orange-50 border border-orange-100 p-2.5 flex items-center gap-2">
              <ArrowDown className="w-4 h-4 text-orange-500 shrink-0" />
              <div className="min-w-0">
                <p className="text-[11px] text-orange-700 font-semibold truncate">#{below.rank} {nameOf(below)}</p>
                <p className="text-[10px] text-orange-600">{below.weeklyXp.toLocaleString("id-ID")} XP</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="mx-4 mt-3 flex items-center gap-2 text-xs text-gray-500">
        ...
        <span>
          <b className="text-gray-900">{payload.totalParticipants.toLocaleString("id-ID")}</b> murid bersaing minggu ini
        </span>
        <span className="mx-1 text-gray-300">•</span>
        <span>
          Season berakhir <b className="text-gray-900"><WeeklyCountdown endsAt={payload.seasonEndsAt} baseline={payload.now} /></b>
        </span>
      </div>

      {/* XP transparency */}
      {activeGroups.length > 0 && (
        <div className="mx-4 mt-4 rounded-xl bg-gray-50 border border-gray-100 p-3">
          <p className="text-xs font-bold text-gray-900 mb-2 flex items-center gap-1.5">
            <ListOrdered className="w-4 h-4 text-violet-500" /> Dari mana XP-mu minggu ini?
          </p>
      ...
      {hallOfFame.length > 0 && (
        <div className="mx-4 mt-4 rounded-xl border border-amber-100 bg-amber-50/50 p-3">
          <p className="text-xs font-bold text-gray-900 mb-2 flex items-center gap-1.5">
            <Crown className="w-4 h-4 text-amber-500" /> Hall of Fame
          </p>
          <div className="space-y-1.5">
            {hallOfFame.slice(0, 3).map((h) => (
              <p key={`${h.periodType}-${h.periodKey}-${h.rank}`} className="text-[11px] text-gray-600 flex items-center gap-1.5">
                ...
                <span className="text-gray-500 shrink-0">{h.score.toLocaleString("id-ID")} XP</span>
              </p>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <div className="flex items-center justify-center gap-4 px-4 py-4 border-t border-gray-100 bg-gray-50/50 flex-wrap">
        <span className="text-xs font-bold text-gray-600 flex items-center gap-1">
          <Flame className="w-3.5 h-3.5 text-orange-500" /> Naikkan peringkatmu!
        </span>
        ...
        <Link href="/arena/league" className="inline-flex items-center gap-1 text-xs font-semibold text-violet-600 hover:text-violet-800 transition-colors">
