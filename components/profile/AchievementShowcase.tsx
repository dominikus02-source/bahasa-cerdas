  ...
  return (
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
      {shown.map((b) => {
        ...
        return (
          <Link
            ...
            className="group flex flex-col items-center gap-1.5 rounded-2xl border border-white/10 bg-white/5 p-3 transition-all hover:border-white/25 hover:bg-white/10"
          >
            <span
      ...
      {Array.from({ length: Math.max(0, max - shown.length) }).map((_, i) => (
        <div
          key={`lock-${i}`}
          className="flex flex-col items-center gap-1.5 rounded-2xl border border-dashed border-white/10 bg-white/5 p-3 opacity-55"
        >
          <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/5 text-white/35">
            <Lock size={18} />
          </span>
