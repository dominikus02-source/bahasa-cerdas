 ...
 *   - /murid/karya   (kanonik, Student Shell) — header opsional via prop
 *   - /arena/feed    (mirror APK + preview guru) — memakai header internal
 * Semua tautan di-parameter-kan agar tiap shell mengarah ke route kanoniknya.
 */
interface KaryaFeedProps {
  /** Ganti header internal ("Jelajah Karya" + CTA Tulis). null → tampilkan header internal. */
  ...
  tulisHref?: string
  /** Route untuk "Hapus Filter" (default "/murid/karya") */
  clearHref?: string
}

function FeedContent({ header, detailBase, profileBase, tulisHref, clearHref }: Required<Pick<KaryaFeedProps, "header" | "detailBase" | "profileBase" | "tulisHref" | "clearHref">>) {
            ...
            return (
              <div key={k.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 overflow-hidden hover:shadow-md transition-all">
                ...
                <div className="flex items-center gap-4 px-5 py-3 border-t border-gray-50 dark:border-slate-800">
                  <button onClick={() => handleLike(k.id)} disabled={likePending[k.id]} className={`flex items-center gap-1.5 text-sm font-medium transition-all ${isLiked ? "text-red-500" : "text-gray-400 hover:text-red-500 dark:text-slate-400"}`}>
                    {isLiked ? <Heart size={16} fill="currentColor" /> : <Heart size={16} />}
                    {likeCount}
                  </button>
                  <Link href={`${detailBase}/${k.id}`} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-violet-500 transition-all dark:text-slate-400 dark:hover:text-violet-300">
                  ...
                  {isOwner(k.user.id) && (
                    <button onClick={() => handleDelete(k.id)} disabled={isDeleting} className="text-gray-300 hover:text-red-500 transition-all dark:text-slate-600">
 ...
 *   - /murid/karya  (kanonik): header disuplai halaman (null → header internal)
 *   - /arena/feed   (mirror): header internal + tautan arena (APK-safe)
 */
