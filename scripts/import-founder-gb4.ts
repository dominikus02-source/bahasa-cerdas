/**
 * FOUNDER BANK GB4 — additive importer for /guru/bank-soal (10 themes, 250 rows).
 *
 * Approved manifest: data/founder-bank-gb4/manifest.json
 * (BC-GB4-<CODE>-<NNN>, source MASTER_BANK, kelas SEMUA — content approved
 ...
 *   - insert missing GB4 Soal rows (idempotent: kodeSoal globally unique,
 ...
 *   - GB4 rows == 250
 *   - zero rows with empty text/key among GB4
 *   - retired count unchanged
 *   - TKAQuestion / PaketKompetensi / TestAnswer / ProgresKompetensi unchanged
 *
 * Safety: dry-run default; --execute requires env FOUNDER_GB4_CONFIRM=YES.
 */
import fs from "fs";
...
const MANIFEST = path.join(process.cwd(), "data/founder-bank-gb4/manifest.json");
const EXECUTE = process.argv.includes("--execute");
const CONFIRMED = process.env.FOUNDER_GB4_CONFIRM === "YES";
const KODE_PREFIX = "BC-GB4-";
...
async function main(): Promise<void> {
  ...
  console.log(`manifest OK: ${rows.length} rows (${pg} PG, ${tf} TF), 250 unique ${KODE_PREFIX} codes`);
  ...
  const beforeGb4 = await db.soal.count({ where: { kodeSoal: { startsWith: KODE_PREFIX } } });
  ...
  console.log(`DB before: active=${beforeActive} retired=${beforeRetired} gb4=${beforeGb4} tka=${beforeTka} paket=${beforePaket} answers=${beforeAnswer} progres=${beforeProgres}`);
  ...
  if (!EXECUTE) {
    console.log("\nDRY RUN ONLY — no DB writes. Re-run with --execute (and FOUNDER_GB4_CONFIRM=YES) to apply.");
    await db.$disconnect();
    return;
  }
  if (!CONFIRMED) throw new Error("EXECUTE requires FOUNDER_GB4_CONFIRM=YES");
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    const gb4Count = await tx.soal.count({ where: { kodeSoal: { startsWith: KODE_PREFIX } } });
    if (gb4Count !== EXPECTED_ROWS) throw new Error(`INVARIANT_FAIL: gb3 ${gb4Count} != ${EXPECTED_ROWS}`);
  ...
  console.log(`\nEXECUTED: inserted ${result.inserted} GB4 rows; active MASTER_BANK ${beforeActive} -> ${result.afterActive}.`);
  console.log("NEXT: add BC-GB4- prefix to delivery gate + verify Guru Bank UI.");
  await db.$disconnect();
}
