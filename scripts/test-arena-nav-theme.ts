...
function main() {
  ...
  test("chat web full-width workspace (w-full, tanpa cap 1440px) — ternary container kini di ArenaWorkspaceContainer",
    () => {
      const container = read("components/arena/workspace-container.tsx");
      return container.includes('pathname.startsWith("/arena/chat")')
        && container.includes('? "w-full py-0 md:px-6"')
        && !arenaLayout.includes("max-w-[1440px]")
        && !container.includes("max-w-[1440px]");
    });
