...
// Check privacy: presence value only stores role, not email/IP/page
...
assert(!heartbeat.includes("req.json()"), "Must not read request body (server derives all)")
assert(!heartbeat.includes("x-forwarded"), "Must not forward/accept IP headers")
assert(!heartbeat.includes("body"), "Must not parse body")
...
assert(!presence.includes("currentPage") && !presence.includes("pageUrl"), "Presence value must not include current page")
