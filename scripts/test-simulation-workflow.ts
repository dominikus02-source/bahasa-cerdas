...
async function main() {
  ...
  test("BIGT page explains perbedaan BC vs BIGT", () => bigtComponent.includes("Perbedaan BahasaCerdas dan BIGT"))
  ...
  test("CertificatePreview does not claim 'Sertifikat resmi' (disclaimer uses 'bukan' prefix)", () => {
    const lines = certPreview.split("\n").filter(l => l.includes("Sertifikat resmi"))
    return lines.every(l => l.includes("bukan"))
  })
