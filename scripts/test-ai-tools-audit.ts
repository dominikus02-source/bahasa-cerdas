/**
 * STEP 5.1.1 — AUDIT SEMUA ALAT AI + RELIABILITY (Soal dan seluruh agent)
...
function main() {
  ...
  // 4. Schema Soal tetap ketat (fixture valid + invalid)
  console.log("\n── 4. Schema Soal ketat (tidak dilemahkan) ──");
  ...
  check("4. fixture soal valid → parse OK", () => soalOutputSchema.safeParse(soalFixture).success);
  check("4. soal tanpa editableText → DITOLAK", () => {
    const bad = { ...soalFixture } as Record<string, unknown>;
    delete bad.editableText;
    return soalOutputSchema.safeParse(bad).success === false;
  });

  // 5. Rantai provider utuh
  console.log("\n── 5. Rantai provider (Groq primary — keputusan founder 5.1.3) ──");
  const provider = read("src/ai/core/provider.ts");
  check("5. default priority = groq only + override AI_PROVIDER_PRIORITY",
    () => provider.includes('return ["groq"];') && provider.includes("AI_PROVIDER_PRIORITY"));
  check("5. Groq rantai model 120b→20b (providerModels)",
    () => provider.includes('"openai/gpt-oss-120b", "openai/gpt-oss-20b"'));
  check("5. providerModels('groq', 'deepseek-chat') = [120b, 20b]",
    () => JSON.stringify(providerModels("groq", "deepseek-chat")) === JSON.stringify(["openai/gpt-oss-120b", "openai/gpt-oss-20b"]));
  check("5. rantai model dipakai di streamProviderText DAN callWithFallback",
    () => provider.includes("providerModels(providerName, req.model)"));
  check("5. DeepSeek model tunggal (deepseek-chat) — tanpa model ganda",
    () => providerModels("deepseek", "deepseek-chat").length === 1);
  check("5. stream kosong melempar ProviderEmptyError (fallback benar-benar jalan)",
    () => provider.includes('if (!fullText.trim()) throw new ProviderEmptyError("deepseek")'));
  check("5. tanpa response_format json_object (prompt JSON-strict)",
    () => !provider.includes("json_object") && !provider.includes("response_format"));

  // 6. Tanpa secret di agent/provider
  console.log("\n── 6. Keamanan ──");
  check("6. tidak ada string key/secret literal di file agent",
    () => Object.values(AGENT_FILES).every((f) => !read(f).includes("sk-") && !read(f).includes("Bearer ")));
  check("6. provider hanya membaca key dari env (process.env, tanpa hardcode)",
    () => !provider.includes("api_key =") && provider.includes("process.env"));

  // 7. Protected zones
  console.log("\n── 7. Protected zones ──");
  check("7. protected zones 0 diff (prisma/gamification/learning-loop/engines/apk/coins/adaptive/learner-state/diagnostic/app-api-player)",
    () => {
  ...
  check("7. perubahan hanya di src/ai + app/api/ai + guru/ai-tools + package.json + scripts (file list)",
    () => {
  ...
  // 8. STEP 5.1.2 — klasifikasi "sibuk" (UNKNOWN_ERROR fix)
  console.log("\n── 8. Klasifikasi error provider 'sibuk' (bukan UNKNOWN_ERROR) ──");
  const runRoute = read("app/api/ai/agents/run/route.ts");
  const runner = read("src/ai/core/agent-runner.ts");
  check("8. route klasifikasi 'sibuk' → PROVIDER_ERROR",
    () => runRoute.includes('err.includes("sibuk")') && runRoute.includes("PROVIDER_ERROR"));
  check("8. runAgent errorCodeFor 'sibuk' → PROVIDER_ERROR",
    () => runner.includes('sibuk/i.test(error)') && runner.includes("PROVIDER_ERROR"));
  check("8. getUserFriendlyMessage 'sibuk' → pesan AI sedang sibuk",
  ...
  // 9. Observability detail chain (server logs)
  console.log("\n── 9. Observability detail provider chain ──");
  check("9. runAgent mencatat detail ProviderChainFailedError ke server logs",
    () => runner.includes("Provider chain failed") && runner.includes("error.errors.slice(0, 12)"));
  check("9. streamProviderText log detail chain gagal",
    () => provider.includes("stream chain failed") && provider.includes("errors.slice(0, 12)"));
  check("9. callWithFallback log detail chain gagal",
    () => provider.includes("call chain failed") && provider.includes("errors.slice(0, 12)"));
  check("9. run route log kegagalan agent (requestId + code + error)",
