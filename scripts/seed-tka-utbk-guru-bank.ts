/**
 * Phase — TKA UTBK & Guru Practice Question Bank Seed
 *
 * Seeds 60 original questions (30 UTBK + 30 Guru) from JSON source files
 * into Prisma TKAQuestion table. Dry-run by default. Use --execute to apply.
 *
 * Run: npx tsx scripts/seed-tka-utbk-guru-bank.ts
 *      npx tsx scripts/seed-tka-utbk-guru-bank.ts --execute
 */
...
const TRACK_CONFIG: Record<string, {
  ...
  TKA_UTBK: {
    type: "TKA_UTBK",
    tingkat: "UMUM",
    title: "Latihan TKA UTBK",
    description: "Latihan soal TKA untuk persiapan UTBK SBMPTN. Terdiri dari 30 soal literasi membaca dengan topik saintek dan soshum. Soal original BahasaCerdas.",
    duration: 120,
    totalQuestions: 30,
    sections: [
      { name: "Seksi I: Membaca", count: 30, seksi: "MEMBACA", timeLimit: 120 },
    ],
  },
  TKA_GURU: {
    type: "TKA_GURU",
    tingkat: "GURU",
    title: "Latihan TKA Guru",
    description: "Latihan soal TKA untuk guru dan profesional. Terdiri dari 30 soal literasi membaca dengan topik pedagogik dan kebahasaan. Soal original BahasaCerdas.",
    duration: 90,
    totalQuestions: 30,
    sections: [
      { name: "Seksi I: Membaca", count: 30, seksi: "MEMBACA", timeLimit: 90 },
    ],
  },
};
...
async function main() {
  console.log("═══════════════════════════════════════════");
  console.log(`  TKA UTBK & Guru Bank — Seed Script`);
  ...
  if (isExecute) {
    ...
    console.log(`  Existing TKA questions: UTBK/UMUM=${existingUtbk}, GURU=${existingGuru}`);
    if (existingUtbk > 0 || existingGuru > 0) {
