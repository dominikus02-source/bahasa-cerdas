  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Karya Siswa</h1>
          <p className="text-sm text-gray-500 mt-1">{total} total karya</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200">
        <div className="p-4 border-b border-slate-100">
          <div className="relative max-w-sm">
            ...
            <input type="text" placeholder="Cari judul karya..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
              className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-violet-500/20" />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-500">Belum ada karya siswa</p>
          </div>
        ) : (
          <>
                  ...
                  {karya.map((k: any) => (
                    <tr key={k.id} className="border-b border-slate-50 hover:bg-slate-50">
                      <td className="py-3 pl-4">
                        <p className="font-medium text-slate-900 truncate max-w-[250px]">{k.title}</p>
                      </td>
                      <td className="py-3 text-xs text-slate-600">{k.user?.fullName || "—"}</td>
                      <td className="py-3">
                      ...
                      <td className="py-3">
                        <div className="flex items-center gap-1">
                          <Link href={`/arena/feed/${k.id}`} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600">
                            <Eye size={14} />
                          </Link>
              ...
              <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100">
                ...
                <div className="flex gap-1">
                  <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                    className="px-3 py-1.5 text-xs rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-50 flex items-center gap-1"><ArrowLeft size={12} /> Sebelumnya</button>
                  <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages}
                    className="px-3 py-1.5 text-xs rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-50 flex items-center gap-1">Selanjutnya <ArrowRight size={12} /></button>
                </div>
              </div>
            )}
          </>
