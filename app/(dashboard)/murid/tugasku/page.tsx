      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {currentList.map(item => (
            <Card key={item.id} className={`p-5 border transition-all ${item.isOverdue ? "border-red-200 bg-red-50/30" : "border-slate-100 dark:border-slate-800 hover:shadow-md"}`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <h3 className="font-bold text-slate-900 dark:text-slate-100">{item.quiz.title}</h3>
                    ...
                    {item.submission && (
                      <span className={`text-xs px-2 py-0.5 rounded-full ${item.submission.status === "GRADED" ? "bg-green-100 text-green-700 dark:text-green-300" : "bg-blue-100 text-blue-700 dark:text-blue-300"}`}>
                  ...
                  {item.quiz.description && (
                    <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-1 mb-2">{item.quiz.description}</p>
                  )}
                  <div className="flex items-center gap-4 text-xs text-slate-400 flex-wrap">
                    ...
                    {item.quiz.topik && <span className="text-violet-600 dark:text-violet-400">{item.quiz.topik}</span>}
                    <span className="flex items-center gap-1">
                      <Calendar className="w-3 h-3" /> Deadline: {formatDate(item.dueDate)}
                    </span>
                    {item.submission && item.submission.score != null && (
                      <span className="font-semibold text-emerald-600 dark:text-emerald-400">Nilai: {Math.round(item.submission.score)}%</span>
                    )}
                  </div>
                  {item.notes && (
                    <p className="text-xs text-slate-500 mt-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-2">{item.notes}</p>
                  )}
                </div>
