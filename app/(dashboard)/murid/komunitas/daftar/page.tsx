  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Bergabung dengan komunitas guru dan siswa Bahasa Indonesia</p>
      </div>
      ...
      <div className="flex gap-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full h-12 pl-10 pr-4 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-violet-500"
          />
        </div>
        <select
          value={filterType}
          onChange={(e) => setFilterType(e.target.value)}
          className="h-12 px-4 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-violet-500"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 font-medium">Belum ada komunitas</p>
          <p className="text-sm text-gray-400 mt-1">Komunitas akan muncul setelah ada yang membuatnya</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((c) => (
            <div key={c.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-lg transition-shadow">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  <span className="text-xs text-violet-600 font-medium">{typeLabels[c.type] || c.type}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Link href={`/komunitas/${c.id}`} className="text-violet-600 text-sm font-medium hover:underline">
                  Lihat →
                </Link>
              </div>
            </div>
