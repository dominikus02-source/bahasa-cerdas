...
function ContentLine({ line, index, ilustrasiUrl }: { line: string; index: number; ilustrasiUrl?: string }) {
  ...
  if (p.type === "salah")
    return (
      <div className="flex items-start gap-2 py-0.5">
        ...
        <span className="text-sm text-red-600 dark:text-red-400">{p.text}</span>
      </div>
    )
  ...
  if (p.type === "bullet")
    return (
      <div className="flex items-start gap-2 py-0.5">
        ...
        <span className="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{p.text}</span>
      </div>
    )
  ...
  if (p.type === "numbered")
    return (
      <div className="flex items-start gap-3 py-0.5">
        <span className="text-xs font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 shrink-0">
          {p.text.match(/^(\d+)/)?.[1] || "•"}
        </span>
        <span className="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{p.text.replace(/^\d+\.\s*/, "")}</span>
      </div>
    )

  if (p.type === "penting")
    return (
      <div className="flex items-start gap-3 p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl my-2">
  ...
  if (p.type === "tip")
    return (
      <div className="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900 rounded-xl my-2">
        <Brain className="w-5 h-5 text-blue-500 dark:text-blue-400 mt-0.5 shrink-0" />
  ...
  if (p.type === "ilustrasi")
    return (
      <div className="my-4 rounded-xl overflow-hidden border border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950/40">
        {ilustrasiUrl ? (
          <img
            src={ilustrasiUrl}
            alt={p.text}
 ...
 <div className="flex items-center gap-2 px-4 py-2.5 bg-white bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
  ...
  return <p className="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{line}</p>
}
...
export default function BelajarPage() {
  ...
  if (loading) return (
    <div className="px-4 py-6 arena-page space-y-4 animate-pulse">
      ...
      <div className="h-24 bg-gray-100 dark:bg-slate-800/80 rounded-2xl" />
      {[1, 2, 3].map(i => <div key={i} className="h-40 bg-gray-100 dark:bg-slate-800/80 rounded-2xl" />)}
    </div>
  )
  ...
  if (!konten) return (
    <div className="px-4 py-6 arena-page text-center pt-20">
      ...
      <p className="text-gray-500 dark:text-slate-400">Materi belum tersedia</p>
 ...
 <div className="sticky top-0 z-10 bg-white bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-b border-gray-100 dark:border-slate-800">
        <div className="px-4 py-3">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-semibold text-violet-600 dark:text-violet-400">{unit?.level?.title || "Belajar"}</p>
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-400 font-medium">{activeIdx + 1} dari {total}</span>
                <span className="text-[11px] text-gray-400">•</span>
                <span className="text-xs font-bold text-violet-600 dark:text-violet-400">{Math.round(((activeIdx + 1) / total) * 100)}%</span>
              </div>
            </div>
          </div>
          <div className="w-full h-2 bg-gray-100 dark:bg-slate-800/80 rounded-full overflow-hidden">
            <div
      ...
      <div className="px-4 py-5 max-w-lg mx-auto">
        ...
        {/* Main Content Card */}
 <div key={activeIdx} className="bg-white dark:bg-slate-800/90 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden animate-fade-in">
          {/* Card Header */}
          <div className="bg-gradient-to-r from-violet-50 to-purple-50 border-b border-violet-100 dark:border-violet-900 px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-base font-bold text-gray-900 dark:text-slate-100">{m.judul}</h2>
            </div>
          </div>
            ...
            {m.contoh && m.contoh.length > 0 && (
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[11px] font-bold text-amber-700 dark:text-amber-300 uppercase tracking-wider">Contoh Penggunaan</span>
                </div>
                <div className="space-y-2">
                  {m.contoh.map((c, ci) => {
                    ...
                    return (
 <div key={ci} className="bg-gray-50 dark:bg-slate-800/60 border border-gray-100 dark:border-slate-800 rounded-xl px-4 py-2.5">
            ...
            {/* Catatan */}
            {m.catatan && (
              <div className="mt-4 flex items-start gap-3 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 dark:border-blue-900 rounded-xl">
                <Sparkles className="w-5 h-5 text-blue-500 dark:text-blue-400 mt-0.5 shrink-0" />
                <div>
                  <span className="text-[11px] font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wider block mb-0.5">Catatan Penting</span>
          ...
          {/* Action Button */}
          <div className="px-5 py-4 bg-gradient-to-b from-white to-gray-50 border-t border-gray-100 dark:border-slate-800">
            <button
              onClick={selesai}
        ...
        {isLast && konten.rangkuman.length > 0 && (
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-5 mt-6 animate-fade-in shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              <Sparkles className="w-5 h-5 text-emerald-500 dark:text-emerald-400" />
              <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider">Rangkuman</span>
            </div>
            <ul className="space-y-2">
