    ...
    return (
      <div className="px-4 py-5 md:px-6 arena-page">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm font-bold text-violet-600">vs {aktif?.lawan.nama}</span>
        </div>
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-5 leading-snug">{s.text}</h2>
        <div className="space-y-3">
          {(s.options as string[]).map((op, i) => (
            <button
              key={i}
              onClick={() => jawab(i)}
              className="w-full text-left px-4 py-3.5 rounded-2xl border-2 border-gray-200 bg-white font-medium text-gray-800 active:scale-[0.98] hover:border-violet-400 hover:bg-violet-50 transition-all"
            >
              {op}
            </button>
          ))}
  ...
  if (view === "hasil" && hasil) {
    return (
      <div className="px-4 py-5 md:px-6 arena-page">
        ...
        <h2 className="font-bold text-gray-900 mb-3">Pembahasan</h2>
        <div className="space-y-3 mb-6">
          {hasil.review.map((r, i) => (
            <div key={i} className={`bg-white rounded-2xl border p-4 ${r.isCorrect ? "border-emerald-200" : "border-rose-200"}`}>
              <div className="flex items-start gap-2">
                {r.isCorrect
                  ? <Check className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                  : <X className="w-5 h-5 text-rose-500 shrink-0 mt-0.5" />}
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 text-sm">{r.text}</p>
                  <p className="text-sm mt-1.5 text-emerald-700">Jawaban: {(r.options as string[])[r.correctAnswer]}</p>
                  {!r.isCorrect && r.myAnswer !== null && (
                    <p className="text-sm text-rose-600">Jawabanmu: {(r.options as string[])[r.myAnswer]}</p>
                  )}
                  {r.explanation && <p className="text-xs text-gray-500 mt-1.5">{r.explanation}</p>}
                </div>
              </div>
            </div>
          ))}
    ...
    return (
      <div className="px-4 py-5 md:px-6 arena-page">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h1 className="text-xl font-extrabold text-gray-900">Pilih Lawan</h1>
        </div>
        ...
        {teman.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
            ...
            <p className="font-semibold text-gray-700">Belum ada teman sekelas</p>
          ...
          <div className="space-y-2.5">
            {teman.map((t) => (
              <button
                ...
                className="w-full flex items-center gap-3 bg-white rounded-2xl border border-gray-100 p-3.5 active:scale-[0.98] hover:border-violet-300 transition-all disabled:opacity-50"
              >
                ...
                <div className="flex-1 text-left min-w-0">
                  <p className="font-bold text-gray-900 truncate">{t.fullName}</p>
                  <p className="text-xs text-gray-400">Tingkat {t.level}</p>
                </div>
                <Swords className="w-5 h-5 text-violet-500 shrink-0" />
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <div className="px-4 py-5 md:px-6 arena-page">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-gray-900">Tantang Teman</h1>
          <p className="text-sm text-gray-500">Duel 10 soal, soal sama, siapa lebih jago?</p>
        </div>
      </div>

      {pesan && (
        <div className="mt-4 flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-semibold rounded-xl px-4 py-3">
      ...
      {loading ? (
        ...
        <>
          {giliranku.length > 0 && (
            <section className="mt-6">
              <h2 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
                <Swords className="w-4 h-4 text-violet-500" /> Giliranmu Main
              </h2>
              <div className="space-y-2.5">
                {giliranku.map((t) => (
                  <button key={t.id} onClick={() => mulaiMain(t)} className="w-full flex items-center gap-3 bg-white rounded-2xl border-2 border-violet-200 p-4 active:scale-[0.98] transition-all">
                    ...
                    <div className="flex-1 text-left min-w-0">
                      <p className="font-bold text-gray-900 truncate">vs {t.lawan.nama}</p>
          ...
          {menunggu.length > 0 && (
            <section className="mt-6">
              <h2 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
                <Clock className="w-4 h-4 text-amber-500" /> Menunggu Lawan
              </h2>
              <div className="space-y-2.5">
                {menunggu.map((t) => (
                  <div key={t.id} className="flex items-center gap-3 bg-white rounded-2xl border border-gray-100 p-4">
          ...
          {riwayat.length > 0 && (
            <section className="mt-6">
              <h2 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
                <Trophy className="w-4 h-4 text-emerald-500" /> Riwayat
              </h2>
              <div className="space-y-2.5">
                {riwayat.map((t) => (
                  <div key={t.id} className="flex items-center gap-3 bg-white rounded-2xl border border-gray-100 p-4">
                    <span className={`px-2.5 py-1 rounded-lg text-xs font-extrabold shrink-0 ${
                      t.hasil === "MENANG" ? "bg-emerald-100 text-emerald-700" : t.hasil === "KALAH" ? "bg-rose-100 text-rose-600" : "bg-amber-100 text-amber-700"
                    }`}>
          ...
          {tantangan.length === 0 && (
            <div className="text-center py-16 mt-4 bg-white rounded-2xl border border-gray-100">
              <div className="w-16 h-16 rounded-full bg-violet-100 flex items-center justify-center mx-auto mb-4">
                <Swords className="w-7 h-7 text-violet-500" />
              </div>
              <p className="font-semibold text-gray-700">Belum ada tantangan</p>
