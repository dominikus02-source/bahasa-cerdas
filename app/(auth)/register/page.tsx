...
export default function RegisterPage() {
  ...
  // GOOGLE ROLE SELECTION (pre-OAuth intent): the user already confirmed an
  // explicit role in this wizard. Seal it server-side into a short-lived intent
  // cookie, then start Google OAuth — the callback consumes the intent exactly
  // once to provision the matching role. Existing accounts ignore the intent.
  const handleGoogleRegister = async () => {
    if (!role || googleLoading) return;
    setGoogleLoading(true);
    setError("");
    try {
      const intentRes = await fetch("/api/auth/role-intent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!intentRes.ok) {
        ...
        setError(data.error || "Gagal menyiapkan pendaftaran Google.");
                  ...
                  <form onSubmit={handleRegister} className="space-y-5">
                    ...
                    <div className="relative">
                      ...
                      <div className="relative flex justify-center text-xs">
                        <span className="bg-white px-3 text-slate-400 dark:bg-slate-900 dark:text-slate-500">atau</span>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="flex h-12 w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-slate-600 dark:hover:bg-slate-800"
                    >
                      {googleLoading ? (
                      ...
                      {googleLoading ? "Memproses..." : `Daftar dengan Google sebagai ${config.label}`}
                    </button>
                  </form>
                )}
